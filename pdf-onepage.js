(() => {
  ...
  function buildPdf() {
    ...
    // The form limits defect text to 550 characters. The density profile below
    // keeps every supported form state on a single DIN-A4 page without dropping content.
    ...
    if (d.defects === 'Keine') {
      ...
      // Fit the complete 550-character defect field into a bounded area.
      // Font size is reduced only when necessary; text is never truncated.
    ...
    // Signature height uses the remaining page space. This is the final safety
    // mechanism that guarantees a one-page PDF for all supported form inputs.
    const labelSpace = 7;
    ...
    // A single-page footer by design; no page-add logic exists in this renderer.
    hLine(282, rule, 0.2);
